...
forcesGenerator = function(actor, aimOverwrite) {
  //Math.random() returns a number between 0 and 1
  var x = player.x;
  var y = player.y;
  var height = 10;
...
updateEntityPosition = function(entity) {
  if (entity.type === "player") {
    if (player.goDown) player.y += 10;
    if (player.goUp) player.y -= 10;
    if (player.goLeft) player.x -= 10;
    if (player.goRight) player.x += 10;
...
//function to launch attack can be applied to any actor in the game
forceAttack = function(actor) {
  if (actor.attackCounter > 25) {
    //every 1 sec
    actor.attackCounter = 0;
    forcesGenerator(actor);
  }
};

//mouse right click produces a special power
document.oncontextmenu = function(mouse) {
  forceSpecialAttack(player);
  mouse.preventDefault();
};
//function to launch special attack witg right click
forceSpecialAttack = function(actor) {
  if (actor.attackCounter > 50) {
    //every 1 sec
    actor.attackCounter = 0;
    //360 forces drawn on canvas
    for (var i = 0; i < 360; i++) {
      forcesGenerator(actor, i);
    }

    // forcesGenerator(actor,actor.aimAngle - 5);
    // forcesGenerator(actor,actor.aimAngle);
    // forcesGenerator(actor,actor.aimAngle + 5);
  }
};
